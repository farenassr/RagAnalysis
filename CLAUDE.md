...
- **FastEndpoints 8.1.0** for APIs — use `Endpoint<EmptyRequest, TRes>` (not `EndpointWithoutRequest<TRes>`) when there's no request body, so `Send.OkAsync(response, ct)` is accessible
